...
npm test -- --runInBand src/app/admin/analytics/__tests__/FinancialReportsPage.test.jsx
```
...
| **Calendar** | `/admin/scheduling-calendar` | Calendar grid, positional slot tracks, month navigation, responsive slot rows, and event/blocking mutations. | Every date reserves Morning/Afternoon/Evening tracks; blocked lines match the red legend; selected-day summary duplication is absent while block/open actions preserve conflict handling. | `PASSED` |
