...
| UI-D022 | Calendar month cells have fixed height and exactly three positional tracks for Morning, Afternoon, and Evening. Tracks stay visually empty until their corresponding period is occupied or blocked; cells show no slot-count or overflow text. | Operators need to identify period availability spatially without reading or decoding per-cell copy. |
...
| UI-D033 | Selected-day slot blocking is presented as three divider-separated rows containing slot name, configured time, status, and one Block/Open action. | Card-per-slot treatment adds visual weight without adding information or capability. |
