...
| UI-D022 | Calendar month cells have fixed height and exactly three positional tracks for Morning, Afternoon, and Evening. Tracks stay visually empty until their corresponding period is occupied or blocked; blocked uses the header legend's red, and cells show no slot-count or overflow text. | Operators need to identify period availability spatially without reading or decoding per-cell copy, and blocked must not be confused with amber Awaiting state. |
...
| UI-D034 | Shared donut charts place the centered chart above full-width legend/data rows, retain a smaller Dashboard variant, and right-align the value column; the Dashboard revenue trend omits its redundant bucket caption. | A vertical composition remains clear at every card width and keeps labels and values aligned beneath the visualization. The chart title and axes already identify the Dashboard trend. |
