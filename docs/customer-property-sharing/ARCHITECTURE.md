...
There is no contact-submission, receipt, media-snapshot, daily-view,
public-file-manifest, raw-view-event, visitor, IP, user-agent, referrer,
fingerprint, or location schema.
...
Public media follows the booking's current accepted browser-safe deliverables.
...
state, selected membership, owner/booking/listing alignment, completion,
current-version identity, acceptance/deletion state, supersession, and a
