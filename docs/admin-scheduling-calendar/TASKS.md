...
| M2 - Calendar and entry flows | `IN_PROGRESS` | 4 | 5 | 5-7 days |
...
| CAL-203 | Integrate full-day and exact time-range blocking | `IN_PROGRESS` | Engineering | CAL-103, permission service | Admins can block `from`/`to` times in 30-minute increments; overlapping customer bookings are rejected; existing records trigger a warning | Existing period-block implementation: `src/app/admin/scheduling-calendar/SchedulingCalendarPage.jsx`, `src/app/api/admin/timeslots/route.js`; exact time-range extension pending |
