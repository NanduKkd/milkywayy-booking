...
export default function DateSlotPicker({
    ...
    useEffect(() => {
        ...
        const fetchAvailability = async () => {
            ...
            try {
                ...
                const data = await getAvailabilityForRange(startStr, endStr);
    ...
    const isSlotAvailable = (startSlotValue, blockedSlots) => {
        const startSlotIndex = TIME_SLOTS.findIndex(s => s.value === startSlotValue);
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="grid grid-cols-1 gap-3">
                                                    {TIME_SLOTS.map((timeSlot) => {
                                                        const isAvailable = isSlotAvailable(timeSlot.value, currentBlockedSlots);
                                                        const isSelectedSlot = slot === timeSlot.value;

                                                        return (
                                                            <button
                                                                key={timeSlot.value}
                                                                onClick={() => isAvailable && onSlotChange(timeSlot.value)}
                                                                ...
                                                                className={cn(
                                                                    ...
                                                                    !isAvailable && "opacity-50 cursor-not-allowed hover:border-zinc-700 bg-[#1a1a1a] text-zinc-600"
                                                                )}
                                                            >
                                                                <span>{timeSlot.label}</span>
                                                                {!isAvailable && <span className="text-[10px] bg-zinc-800 px-2 py-0.5 rounded text-gray-400">Unavailable</span>}
                                                                {isSelectedSlot && <span className="w-2 h-2 rounded-full bg-black"></span>}
                                                            </button>
                                                        );
                                                    })}
                                                </div>
