...
export const savePricings = async(form) => {
  ...
  const existing = await models.DynamicConfig.findOne({ where: { key: 'pricings' } })
  if(existing)
    existing.update({ value: sections });
  else {
...
export const getPricings = async() => {
  // await wait(3000);
  const configEntry = await models.DynamicConfig.findOne({ where: { key: 'pricings' } })
