...
export default function BookNew({
  ...
  const onContinue = async (data) => {
    ...
    try {
      // Save bookings to DB
      const ids = await createBookings(data.properties);
      setBookingIds(ids);
      setStep("payment");
  ...
  const handleApplyCoupon = async () => {
    ...
    const result = await validateCoupon(couponCode, finalAmount);
  ...
  useEffect(() => {
    if (couponCode && discountAmount > 0) {
      ...
      validateCoupon(couponCode, amountAfterAuto).then((result) => {
        if (result.valid) {
          setDiscountAmount(result.discount);
        } else {
          setDiscountAmount(0);
                        ...
                        <CardBody className="pt-6 space-y-8 overflow-visible">
                          ...
                          <div>
                            <DateSlotPicker
                              ...
                              duration={getPropertyDurationAndEvening(property).duration}
                              allowEvening={getPropertyDurationAndEvening(property).allowEvening}
                  ...
                  <CardBody className="space-y-4">
                    ...
                    <div className="pt-4">
                      <Input
                        ...
                        classNames={{
                          inputWrapper:
                            "bg-[#272727] border-zinc-700 hover:border-zinc-500 focus-within:!border-white",
                          input: "text-white",
